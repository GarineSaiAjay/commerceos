...
- `docker compose -f infra/docker-compose.yml up -d --build` running. This one command is the whole setup: it applies migrations, seeds the catalog, operator account, reviews, and policy settings (`db/seeds/00{1,2,3,4}_*.sql`), and starts both `backend` and `frontend` — nothing else needs to be started or configured.
- Use the frontend at `http://localhost:3000` (the container docker compose just started, kept up by `restart: unless-stopped`) — dashboard tabs open in a second window/tab: `/dashboard`, `/dashboard/runs`, `/dashboard/safety`, `/dashboard/campaigns`. **Don't also run `cd frontend && npm run dev`** — Next.js will find port 3000 already taken and silently move to `http://localhost:3001` instead, which is not the origin a judge running the one-line docker compose command would land on.
- Razorpay Test Mode keys configured (`infra/.env`) — every payment beat below is a **real** Razorpay Test Mode API call, not a mock.
- Nothing to pre-create for the Authorization beat: the checkout flow calls `POST /policy/mandates` itself, live, right before every payment attempt (`usePaymentFlow.ts`), with `maximum_amount` always set equal to that cart's own subtotal — so the mandate itself never gates anything. What actually produces the Level 2/3 routing below is the merchant's policy ceiling (₹30,000, seeded by `docker compose up` above) compared against the platform's fixed amount thresholds (`routeLevel` in `backend/policy/engine.go`): auto-approve ≤ ₹1,000, confirm ₹1,001–₹10,000, hard gate above ₹10,000. Just make sure the cart in beat 2:00 is over ₹10,000, which the AirPods Pro + Case combo below already is.
...
**If there's no live presenter** (the checkout page's "Guided demo" toggle): it packages beats 1:00–4:30 above (ask the agent, accept its proposal, see the cross-sell, go over budget, watch the graceful rejection, read the audit trail) as an on-screen checklist a judge can drive themselves, with a one-line hint for whichever step comes next (see `frontend/app/checkout/DemoGuide.tsx`). It's a UI presentation of the same real flow this whole script walks through live — not a scripted mock of it — so anything true of this script is true of what the checklist tracks. It doesn't cover the payment/webhook or Campaign Orchestrator beats (2:30–5:00), which need a live Razorpay Test Mode checkout and pre-seeded rejected demand respectively.
